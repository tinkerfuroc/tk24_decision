...
def createSearchForBin() -> pytree.composites.Sequence:
    ...
    goto_1m_pos = BtNode_Goto("Goto 1m pos", None, target=pose_1m)

    scan_for_bin = BtNode_ScanAndSave("Scan for bin", None, KEY_POINT_BIN_REL, "temp", object=PROMPT_TRASH_BIN)

    save_bin = BtNode_RelToAbs("Save absolute bin position", KEY_POINT_BIN_REL, KEY_POINT_BIN_ABS, True)
...
def createScanAndTurn():
    ...
    scan = BtNode_ScanAndSave("Scan & check if found", None, KEY_POINT_TRASH, KEY_TYPE_TRASH, object=PROMPT_ALL)
    # scan = pytree.behaviours.Running("running")

    turn = BtNode_Goto("turn 90 degrees", None, target=pose_turn90)
...
def create_drop_node():
    ...
    goto_bin = BtNode_GotoGrasp("Got to Trashcan", KEY_POINT_BIN_ABS)

    find_bin = BtNode_ScanAndSave("Find trash can", None, KEY_POINT_BIN_REL, "temp", object=PROMPT_TRASH_BIN)

    # TODO: replace with actual functioning node
    drop = BtNode_Drop("Drop in bin", KEY_POINT_BIN_REL)
...
def searchAndPickupAtBin():
    ...
    goto_trash = BtNode_GotoGrasp("Got to trash point", KEY_POINT_TRASH)

    grasp = BtNode_Grasp("Grasp trash", KEY_TYPE_TRASH)

    drop = create_drop_node()

    root.add_children([goto_bin, search, goto_trash, grasp, drop])
    # root.add_children([search, goto_trash, pytree.behaviours.Running("running")])
...
def searchAndPickupOutside():
    ...
    scan = BtNode_ScanAndSave("Scan for trash", None, KEY_POINT_TRASH, KEY_TYPE_TRASH, object=PROMPT_ALL)
